...
const ProductCard: React.FC<ProductCardProps> = ({ product, viewMode = "grid" }) => {
  ...
  if (viewMode === "list") {
    ...
    return (
      <Link
        ...
        className="group bg-white rounded-xl border border-gray-200 hover:border-gray-300 transition-all duration-300 hover:shadow-xl overflow-hidden block"
  ...
  if (viewMode === "compact") {
    ...
    return (
      <Link
        href={`/urun/${productSlug}`}
        className="group bg-white rounded-lg border border-gray-200 hover:border-gray-300 transition-all duration-300 hover:shadow-lg overflow-hidden block"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        {/* Image Section */}
